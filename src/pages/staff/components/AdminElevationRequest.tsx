...
export function AdminElevationRequest({ onSuccess }: AdminElevationRequestProps) {
    ...
    const [requestElevation, { isLoading, isSuccess, isError, error }] = useRequestAdminElevationMutation();
        ...
        return (
            <div className="p-10 flex flex-col items-center text-center gap-6 animate-in zoom-in-95 duration-500">
                ...
                <div className="flex flex-col gap-2">
                    <h3 className="text-xl font-black uppercase tracking-tight text-foreground">Request Submitted</h3>
                    <p className="text-sm font-bold text-muted max-w-xs italic">Your application for administrative access is now under review. You will be notified once a decision is made.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-8 p-10 animate-in fade-in duration-700">
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-3 mb-2 text-primary">
                    <Shield size={24} />
                    <h2 className="text-2xl font-black uppercase tracking-tight">Administrative Access</h2>
                </div>
                <p className="text-xs font-bold text-muted leading-relaxed">Please provide a clear justification for why you require administrative privileges. This request will be reviewed by the current admin team.</p>
            </div>
