  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs font-medium leading-relaxed">
            You are about to <span className="font-bold">{type.toLowerCase()}</span> the{' '}
            {request?.type.toLowerCase()} request from{' '}
            <span className="font-bold italic">{request?.username}</span>.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        />
        <p className="text-[10px] text-muted font-bold italic">
          This comment will be included in the notification email sent to the employee.
        </p>
      </div>
