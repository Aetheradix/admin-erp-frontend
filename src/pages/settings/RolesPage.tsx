...
const initialRoles: Role[] = [
  {
    ...
    users: initialUsers.filter((u) => u.role === 'Super Admin').length,
    permissions: {
      users: true,
      projects: true,
      finance: true,
  ...
  },
  {
    ...
    users: initialUsers.filter((u) => u.role === 'Admin').length,
    permissions: {
      users: true,
      projects: true,
      finance: true,
  ...
  },
  {
    ...
    users: initialUsers.filter((u) => u.role === 'Manager').length,
    permissions: {
      users: true,
      projects: true,
      finance: false,
  ...
  {
    name: 'Developer',
    description: 'Project and task access',
    users: initialUsers.filter((u) => u.role === 'Developer').length,
    permissions: {
      users: false,
      projects: true,
      finance: false,
  ...
  },
  {
    ...
    users: initialUsers.filter((u) => u.role === 'Viewer').length,
    permissions: {
      users: false,
      projects: false,
      finance: false,
  ...
  const columns: ColumnsType<Role> = [
    {
      ...
      render: (_: unknown, record: Role) => (
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-lg bg-error/5 flex items-center justify-center text-error border border-error/10">
            <Shield size={18} />
          </div>
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-medium text-muted-foreground leading-tight italic">
              {record.description}
            </span>
          </div>
        </div>
    ...
    },
    {
      ...
      key: 'users',
      render: (users) => (
        <div className="px-3 py-1 bg-surface-subtle rounded-lg text-[10px] font-black inline-block uppercase text-muted tracking-widest">
          {users} users
        </div>
      ),
    },
      ...
      >
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Configure global visibility settings for sidebar sections and limit the maximum number
            of sections displayed.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-border-subtle/50">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <span className="text-xs font-extrabold text-muted uppercase tracking-[0.1em]">
                Max display count
              </span>
              <span className="px-3 py-1 bg-primary/10 text-primary font-black rounded-lg text-xs">
                {sectionConfig.maxSections} Sections
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setSectionConfig({ ...sectionConfig, maxSections: parseInt(e.target.value) })
              }
              className="w-full accent-primary h-2 bg-surface-subtle rounded-lg appearance-none cursor-pointer"
            />
            <span className="text-[10px] text-muted italic">
              Limits the count of displayed navigation items in the sidebar.
            </span>
          </div>
