...
export function ApiTokenModal({ visible, onHide }: ApiTokenModalProps) {
  ...
  const handleCreateToken = (e: React.FormEvent) => {
    ...
    if (!tokenName.trim()) {
      showToast({
        ...
        detail: 'Please provide a name for the API token.',
      });
      return;
    }
    ...
    showToast({
      ...
      detail: `API Token "${tokenObj.name}" created successfully.`,
    });
  };
  ...
  const handleCopy = (text: string) => {
    ...
    showToast({
      severity: 'info',
      summary: 'Copied',
      detail: 'Secret API token copied to clipboard.',
    });
    ...
    showConfirm({
      ...
      accept: () => {
        ...
        showToast({
          ...
          detail: `API Token "${name}" has been permanently revoked.`,
        });
      },
    });
  };
