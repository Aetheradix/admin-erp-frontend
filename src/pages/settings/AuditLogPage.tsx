...
export function AuditLogPage() {
  ...
  const columns: ColumnsType<AuditLog> = [
    ...
    },
    {
      ...
      key: 'target',
      render: (text) => (
        <span className="px-3 py-1 bg-primary/5 rounded-lg text-[10px] font-bold text-primary tracking-wide">
          {text}
        </span>
      ),
    },
    ...
    },
    {
      ...
      key: 'status',
      render: (status) => (
        <span className={`text-xs font-black uppercase tracking-tight ${statusColors[status]}`}>
          {status}
        </span>
      ),
    },
  ...
  return (
    <div className="flex flex-col gap-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-white rounded-2xl border border-border-subtle shadow-soft p-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="px-4 py-2 bg-surface-subtle rounded-md text-[10px] font-black uppercase tracking-widest text-muted">
            All Time
          </div>
          <div className="px-4 py-2 bg-surface-subtle rounded-md text-[10px] font-black uppercase tracking-widest text-muted">
            Filter
