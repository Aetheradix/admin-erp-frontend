...
const initialExpenses: Expense[] = [
    { id: 1, category: 'Hardware', description: 'MacBook Pro 16" - Engineering', amount: 2499, status: 'Approved' },
    { id: 2, category: 'Food & Drink', description: 'Team Lunch - Q2 Planning', amount: 120, status: 'Pending' },
    { id: 3, category: 'Utilities', description: 'Office Electricity - May', amount: 450, status: 'Approved' },
    { id: 4, category: 'Subscriptions', description: 'AWS Infrastructure', amount: 1560, status: 'Pending' },
];
...
export function ExpensesPage() {
    ...
    return (
        <div className="flex flex-col gap-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <PageHeader title="Expenses" description="Track and manage organizational spending."
                breadcrumbs={[{ label: 'Home', url: '/' }, { label: 'Finance', url: '/finance' }, { label: 'Expenses' }]}
                primaryAction={{ label: 'Record Expense', onClick: () => setShowForm(true), icon: 'pi pi-plus' }}
            />

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {expenses.map((exp, i) => (
                    <motion.div key={exp.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}
                        ...
                        <div className="flex items-start justify-between mb-6">
                            ...
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${exp.status === 'Approved' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>{exp.status}</span>
                        </div>
                        <div className="flex flex-col gap-1 mb-4">
                            <h3 className="text-lg font-black text-foreground tracking-tight leading-tight">{exp.description}</h3>
            ...
            <Dialog visible={showForm} onHide={() => setShowForm(false)} header="Record Expense" modal
            ...
            >
                <div className="flex flex-col gap-5">
                    <div className="flex flex-col gap-2">
                        ...
                        <Input placeholder="e.g. Team lunch for Q2 planning" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-border-subtle">
                        ...
                        <Button label="Record Expense" onClick={handleSubmit} icon="pi pi-check" className="rounded-xl! px-8!" />
                    </div>
                </div>
            </Dialog>
        </div>
