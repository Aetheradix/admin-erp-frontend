    ...
    <div className="group bg-white rounded-4xl border border-border-subtle overflow-hidden hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 flex flex-col h-full">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent flex items-end p-6">
          <Badge
            ...
            className="rounded-full px-4 py-1! text-[10px] font-black tracking-widest uppercase border-none backdrop-blur-md bg-white/20! text-white!"
          >
            {event.category}
          </Badge>
        </div>
      ...
      <div className="p-6 flex flex-col flex-1 gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted font-medium line-clamp-2">{event.description}</p>
        </div>
        ...
        <div className="mt-auto grid grid-cols-2 gap-y-3 pt-4 border-t border-border-subtle/50">
          ...
          <div className="flex items-center gap-2 text-foreground font-bold text-xs uppercase tracking-tighter">
            ...
            <span className="truncate">
              {event.location?.split(',')[0] || event.location || 'Remote'}
            </span>
          </div>
        ...
        <Button
          variant="secondary"
          className="w-full mt-2 h-12 rounded-3xl! gap-2 font-black text-xs uppercase tracking-widest border-border-subtle! group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all"
        >
          Register Now
        </Button>
      </div>
