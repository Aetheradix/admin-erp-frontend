...
export function AttendanceCalendar({
  ...
  const getDayTypeStyles = (type: string, status?: string) => {
    ...
    switch (type) {
      ...
      case 'Work from home':
        return isPending
          ? 'bg-info/5 text-info/50 border-info/10 border-dashed'
          : 'bg-info/10 text-info border-info/20';
      case 'On Leave':
      case 'Day Off/Leave':
      case 'Sick Leave':
        return isPending
          ? 'bg-warning/5 text-warning/50 border-warning/10 border-dashed'
          : 'bg-warning/10 text-warning border-warning/20';
      case 'Holiday':
        return 'bg-muted/10 text-muted border-muted/20';
      default:
        return 'bg-surface-subtle border-transparent';
    }
  };
    ...
    <div className="bg-white rounded-[40px] border border-border-subtle shadow-soft overflow-hidden animate-in fade-in duration-700">
      ...
      <div className="p-8 flex items-center justify-between border-b border-border-subtle/50">
        <div className="flex flex-col">
          ...
          <span className="text-xs font-bold text-muted uppercase tracking-widest italic">
            Personal Attendance Journey
          </span>
        </div>
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: offset }).map((_, i) => (
          <div
            key={`empty-${i}`}
            className="h-32 border-r border-b border-border-subtle/10 bg-surface-subtle/10"
          />
        ))}
                  ...
                  >
                    ...
                    {'checkIn' in displayItem && displayItem.checkIn && (
                      <span className="opacity-60">
                        {displayItem.checkIn} - {displayItem.checkOut}
                      </span>
                    )}
                  </div>
                )}
