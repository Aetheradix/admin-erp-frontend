            ...
            try {
                ...
                if (hostUrl && email && apiToken && projectKey) {
                    ...
                    if (response.ok) {
                        ...
                        finalDescription = (form.description ? form.description + '\n\n' : '') + `🔗 **Jira Issue:** [${issueKey}](${issueLink})`;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.map((task, i) => {
                    const renderDescriptionWithLinks = (text: string) => {
                        ...
                        return <p className="text-xs text-muted-foreground whitespace-pre-line leading-relaxed pt-1 line-clamp-3">{text}</p>;
                    };
                    ...
                    {isJiraConnected && jiraConfigStr && (
                        <div className="flex items-center gap-3 p-4 rounded-xl bg-surface-subtle border border-border-subtle mt-2">
                            ...
                            <div className="flex flex-col gap-0.5">
                                <label htmlFor="syncToJira" className="text-xs font-bold text-foreground cursor-pointer select-none">
                                    Sync Task with Jira
                                </label>
                                <span className="text-[10px] text-muted italic">Automatically generates a corresponding issue in Jira Cloud when created.</span>
                            </div>
                        </div>
                    )}
