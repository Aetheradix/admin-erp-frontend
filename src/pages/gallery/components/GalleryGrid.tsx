...
export const GalleryGrid = ({ items, onEdit, onDelete }: GalleryGridProps) => {
  ...
  const itemTemplate = (item: GalleryItem) => {
    return (
      <div className="flex flex-col items-center relative gap-6">
        ...
        <img src={item.url} alt={item.title} style={{ width: '100%', display: 'block', maxHeight: '75vh', objectFit: 'contain' }} className="rounded-3xl shadow-2xl" />
  ...
  const thumbnailTemplate = (item: GalleryItem) => {
    return <img src={item.url} alt={item.title} style={{ width: '60px', borderRadius: '8px' }} />;
  };
