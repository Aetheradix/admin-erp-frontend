...
const EMOJI_ICON_MAP: Record<string, { icon: LucideIcon; bg: string; color: string }> = {
    '📊': { icon: BarChart3, bg: 'bg-emerald-500/10 dark:bg-emerald-500/20', color: 'text-emerald-600 dark:text-emerald-400' },
    '⏱️': { icon: Clock, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-600 dark:text-amber-400' },
    '🕒': { icon: Clock, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-600 dark:text-amber-400' },
    '👥': { icon: Users, bg: 'bg-indigo-500/10 dark:bg-indigo-500/20', color: 'text-indigo-600 dark:text-indigo-400' },
    '👤': { icon: Users, bg: 'bg-indigo-500/10 dark:bg-indigo-500/20', color: 'text-indigo-600 dark:text-indigo-400' },
    '📋': { icon: CheckSquare, bg: 'bg-blue-500/10 dark:bg-blue-500/20', color: 'text-blue-600 dark:text-blue-400' },
    '📝': { icon: FileText, bg: 'bg-blue-500/10 dark:bg-blue-500/20', color: 'text-blue-600 dark:text-blue-400' },
    '💳': { icon: CreditCard, bg: 'bg-purple-500/10 dark:bg-purple-500/20', color: 'text-purple-600 dark:text-purple-400' },
    '📦': { icon: Package, bg: 'bg-orange-500/10 dark:bg-orange-500/20', color: 'text-orange-600 dark:text-orange-400' },
    '⚙️': { icon: Settings, bg: 'bg-slate-500/10 dark:bg-slate-500/20', color: 'text-slate-600 dark:text-slate-400' },
    '🚀': { icon: Rocket, bg: 'bg-rose-500/10 dark:bg-rose-500/20', color: 'text-rose-600 dark:text-rose-400' },
    '💡': { icon: Lightbulb, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-500' },
    '⚡': { icon: Zap, bg: 'bg-yellow-500/10 dark:bg-yellow-500/20', color: 'text-yellow-600 dark:text-yellow-400' },
    '🔍': { icon: Search, bg: 'bg-sky-500/10 dark:bg-sky-500/20', color: 'text-sky-600 dark:text-sky-400' },
    '📈': { icon: TrendingUp, bg: 'bg-teal-500/10 dark:bg-teal-500/20', color: 'text-teal-600 dark:text-teal-400' },
    '🔒': { icon: Lock, bg: 'bg-violet-500/10 dark:bg-violet-500/20', color: 'text-violet-600 dark:text-violet-400' },
    '🎯': { icon: Target, bg: 'bg-red-500/10 dark:bg-red-500/20', color: 'text-red-600 dark:text-red-400' },
    '✨': { icon: Sparkles, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-500' },
    '✅': { icon: CheckCircle2, bg: 'bg-emerald-500/10 dark:bg-emerald-500/20', color: 'text-emerald-600 dark:text-emerald-400' },
    '⚠️': { icon: AlertTriangle, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-600 dark:text-amber-400' },
    '❌': { icon: XCircle, bg: 'bg-red-500/10 dark:bg-red-500/20', color: 'text-red-600 dark:text-red-400' },
    '🔑': { icon: Key, bg: 'bg-yellow-500/10 dark:bg-yellow-500/20', color: 'text-yellow-600 dark:text-yellow-400' },
    '🌐': { icon: Globe, bg: 'bg-cyan-500/10 dark:bg-cyan-500/20', color: 'text-cyan-600 dark:text-cyan-400' },
    '💼': { icon: Briefcase, bg: 'bg-indigo-500/10 dark:bg-indigo-500/20', color: 'text-indigo-600 dark:text-indigo-400' },
    '🏆': { icon: Award, bg: 'bg-amber-500/10 dark:bg-amber-500/20', color: 'text-amber-500' },
    '📌': { icon: Pin, bg: 'bg-rose-500/10 dark:bg-rose-500/20', color: 'text-rose-600 dark:text-rose-400' },
};
...
function renderInlineWithIcons(text: string, isUser?: boolean): React.ReactNode[] {
    ...
    const regex = new RegExp(`(${emojiKeys.map((k) => k.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')).join('|')})`, 'g');
...
export const FormattedChatMessage: React.FC<FormattedChatMessageProps> = ({ content, isUser }) => {
    if (isUser) {
        return <div className="whitespace-pre-wrap font-sans">{renderInlineWithIcons(content, true)}</div>;
    }
    ...
    lines.forEach((line, lineIndex) => {
        ...
        if (trimmed.startsWith('###') || (trimmed.startsWith('**"') && trimmed.endsWith('"**'))) {
            const title = trimmed.replace(/^###\s*/, '').replace(/^\*\*/, '').replace(/\*\*$/, '').replace(/^"/, '').replace(/"$/, '');
            elements.push(
                <div
                    key={lineIndex}
