...
export const ARXLogo: React.FC<ARXLogoProps> = ({
  ...
  const hasInitialPulsedRef = React.useRef(false);

  React.useEffect(() => {
    if (isCheckedIn && !isOnBreak) {
      // Trigger 5-second green pulse timer on check-in
      if (!hasInitialPulsedRef.current) {
        setIsPulsingGreen(true);
        hasInitialPulsedRef.current = true;
        const timer = setTimeout(() => {
          setIsPulsingGreen(false);
        }, 5000);
      ...
      setIsPulsingGreen(false);
      if (!isCheckedIn) {
        hasInitialPulsedRef.current = false;
      }
    }
      ...
      >
        ...
        >
          ...
          <style>
            {`
              @keyframes arxGreenPulse {
                0%, 100% { fill-opacity: 1; filter: drop-shadow(0 0 4px rgba(34, 197, 94, 0.9)); }
                50% { fill-opacity: 0.25; filter: drop-shadow(0 0 0px rgba(34, 197, 94, 0)); }
              }
              @keyframes arxYellowPulse {
                0%, 100% { fill-opacity: 1; filter: drop-shadow(0 0 4px rgba(234, 179, 8, 0.9)); }
                50% { fill-opacity: 0.25; filter: drop-shadow(0 0 0px rgba(234, 179, 8, 0)); }
              }
              .arx-status-green  { animation: arxGreenPulse  1.2s cubic-bezier(0.4, 0, 0.6, 1) infinite; }
          ...
          {/* Center status triangle:
              ...
              🟢 Green 5s pulse → initial check-in (5 sec duration)
              🟢 Solid Green     → checked-in (after 5 sec pulse)
